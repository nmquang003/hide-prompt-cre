...
class General_Prompt(nn.Module):
    def __init__(self, args):
        super().__init__()
    ...
    def forward(self, x_embed, x_key=None):
        ...
        prompt_key_norm = nn.functional.normalize(self.prompt_key[1:,], dim=1)
        x_key_norm = nn.functional.normalize(x_key, dim=1)

        similarity = torch.matmul(x_key_norm, prompt_key_norm.t())
        
        softmax_sim = F.softmax(similarity, dim=1) * (1 - self.args.weight_general_prompt)
        ones_column = self.args.weight_general_prompt * torch.ones((softmax_sim.shape[0], 1), device=softmax_sim.device)
        
        softmax_sim = torch.cat([ones_column, softmax_sim], dim=1)

        
        _, _id = torch.topk(similarity, k=self.top_k-1, dim=1)
