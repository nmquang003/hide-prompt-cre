...
class Manager(object):
    ...
    def cal_consine_description(self, args, encoder, all_description):
        ...
        rel2id = dict()
        with torch.no_grad():
            for idx, (rel, descriptions) in enumerate(all_description.items()):
                rel2id[idx] = self.rel2id[rel]
                temp = []
                for description in descriptions:
                    des_tokens = torch.tensor([description['token_ids']]).to(args.device)
                ...
                temp = temp.squeeze(0)
                description_out[self.rel2id[rel]] = temp
                description_matrix[self.rel2id[rel]] = temp
                
        description_matrix = torch.stack(description_matrix, dim=0)
    
        # Tính cosine similarity giữa reps và descriptions
        similarities = sim(description_matrix, description_matrix)
        
        # Chuyển tensor thành numpy array để lưu
        similarities_np = similarities.numpy()
        
        # Tạo thư mục lưu file nếu chưa có
        folder_name = "Similarity_Matrices"
        os.makedirs(folder_name, exist_ok=True)

        # Lưu ma trận dưới dạng JSON
        json_filename = f"{folder_name}/cosine_similarity_matrix.json"
        with open(json_filename, "w") as f:
            json.dump(similarities_np.tolist(), f, indent=2)
    ...
    def train(self, args):
        ...
        for steps, (training_data, valid_data, test_data, current_relations, 
            ...
            if steps == 9:
                self.cal_consine_description(args, encoder, seen_descriptions)
